 ...
 $.fn.dlshouwen.grid = {
  ...
  init : function(options, name) {
   ...
   var gridObject = {
     ...
     if (gridReflectionObj.init.toolsIsInit == false) {
      ...
      if (!gridReflectionObj.option.ajaxLoad) {
       gridReflectionObj.hideProcessBar(function() {
        ...
        // 获取展现数据集
        gridReflectionObj.exhibitDatas = gridReflectionObj.baseDatas.slice(gridReflectionObj.pager.startRecord,
          gridReflectionObj.pager.startRecord + gridReflectionObj.pager.pageSize);
        // 获取排序数据集备份
     ...
     if (!gridReflectionObj.option.ajaxLoad || gridReflectionObj.option.loadAll) {
      gridReflectionObj.hideProcessBar(function() {
       ...
       if (gridReflectionObj.fastQueryParameters || gridReflectionObj.advanceQueryParameter) {
        ...
        if (gridReflectionObj.fastQueryParameters) {
         gridReflectionObj.baseDatas = gridReflectionObj.doFastQueryDatasFilter(gridReflectionObj.baseDatas,
           gridReflectionObj.fastQueryParameters);
        }
        // 处理高级查询
        if (gridReflectionObj.advanceQueryParameter) {
         gridReflectionObj.baseDatas = gridReflectionObj.doAdvanceQueryDatasFilter(gridReflectionObj.baseDatas,
           gridReflectionObj.advanceQueryParameter);
        }
       }
       // 记录数、页数重算
       gridReflectionObj.pager.recordCount = gridReflectionObj.baseDatas.length;
       gridReflectionObj.pager.pageCount = gridReflectionObj.pager.recordCount == 0 ? 0 : (Math
         .floor((gridReflectionObj.pager.recordCount - 1) / gridReflectionObj.pager.pageSize) + 1);
      ...
      $.ajax({
       ...
       success : function(pager) {
        //处理onSessionTimeout{
        var isLoginPage=pager.indexOf("loginPage")!=-1;
        if(isLoginPage){
    ...
    constructGrid : function() {
     ...
     // 构件表头
     var gridContent = '';
     gridContent += '<table class="dlshouwen-grid ' + gridReflectionObj.option.tableClass + '" id="dlshouwen_grid_'
       + gridReflectionObj.option.id + '" style="' + gridReflectionObj.option.tableStyle + '">';
     if (gridReflectionObj.option.showHeader != false) {
      ...
      if (gridReflectionObj.option.check) {
       if (gridReflectionObj.option.checkWidth != null) {
        gridContent += '	<th class="check-column" width="' + gridReflectionObj.option.checkWidth
          + '"><input type="checkbox" id="dlshouwen_grid_' + gridReflectionObj.option.id + '_check" value="check"></th>';
       } else {
        gridContent += '	<th class="check-column"><input type="checkbox" id="dlshouwen_grid_' + gridReflectionObj.option.id
          + '_check" value="check"></th>';
       }
      }
      for (var i = 0; i < columns.length; i++) {
       if (columns[i].width != null) {
        gridContent += '	<th width="' + columns[i].width + '" columnNo="' + i + '" columnId="' + columns[i].id
          + '" class="dlshouwen-grid-header ' + gridReflectionObj.getColumnClassForHide(columns[i]) + ' ' + columns[i].headerClass
          + ' can-sort" style="' + columns[i].headerStyle + '">';
       } else {
        gridContent += '	<th columnNo="' + i + '" columnId="' + columns[i].id + '" class="dlshouwen-grid-header '
          + gridReflectionObj.getColumnClassForHide(columns[i]) + ' ' + columns[i].headerClass + ' can-sort" style="'
          + columns[i].headerStyle + '">';
       }
       if (gridReflectionObj.sortParameter && gridReflectionObj.sortParameter.columnId
         && gridReflectionObj.sortParameter.columnId == columns[i].id) {
        if (gridReflectionObj.sortParameter.sortType == '1') {
         gridContent += '<span class="dlshouwen-grid-sort">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].sortColumn.asc
           + '</span>';
        }
        if (gridReflectionObj.sortParameter.sortType == '2') {
         gridContent += '<span class="dlshouwen-grid-sort dlshouwen-grid-sort-desc">'
           + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].sortColumn.desc + '</span>';
        }
       }
     ...
     if (gridReflectionObj.exhibitDatas != null) {
      ...
      for (var i = 0; i < gridReflectionObj.exhibitDatas.length; i++) {
       ...
       if (gridReflectionObj.option.extraWidth != null) {
        gridContent += '	<td class="extra-column extra-column-event ' + extraColumnClass + '" width="'
          + gridReflectionObj.option.extraWidth + '" dataNo="' + i + '">'
          + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].extraColumn.open + '</td>';
       } else {
        gridContent += '	<td class="extra-column extra-column-event ' + extraColumnClass + '" dataNo="' + i + '">'
          + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].extraColumn.open + '</td>';
       }
       if (gridReflectionObj.option.check) {
        if (gridReflectionObj.option.checkWidth != null) {
         gridContent += '	<td class="check-column text-center" width="' + gridReflectionObj.option.checkWidth
           + '"><input type="checkbox" dataNo="' + i + '" id="dlshouwen_grid_' + gridReflectionObj.option.id + '_check_' + i
           + '" class="dlshouwen-grid-check" value="' + i + '"></td>';
        } else {
         gridContent += '	<td class="check-column text-center"><input type="checkbox" dataNo="' + i + '" id="dlshouwen_grid_'
           + gridReflectionObj.option.id + '_check_' + i + '" class="dlshouwen-grid-check" value="' + i + '"></td>';
        }
       }
       var columns = gridReflectionObj.option.columns;
       for (var j = 0; j < columns.length; j++) {
        if (columns[j].width != null) {
         gridContent += '	<td width="' + columns[j].width + '" dataNo="' + i + '" columnNo="' + j + '" class="dlshouwen-grid-cell '
           + gridReflectionObj.getColumnClassForHide(columns[j]) + ' ' + columns[j].columnClass + '" style="' + columns[j].columnStyle
           + '">';
        } else {
         gridContent += '	<td dataNo="' + i + '" columnNo="' + j + '" class="dlshouwen-grid-cell '
           + gridReflectionObj.getColumnClassForHide(columns[j]) + ' ' + columns[j].columnClass + '" style="' + columns[j].columnStyle
           + '">';
        }
       ...
       gridContent += '	</tr>';
       gridContent += '	<tr id="dlshouwen_grid_' + gridReflectionObj.option.id + '_extra_columns_' + i
         + '" class="dlshouwen-grid-extra-columns hidden">';
       gridContent += '		<td dataNo="' + i + '" colspan="' + (columns.length + 1 + (gridReflectionObj.option.check ? 1 : 0)) + '">';
       for (var j = 0; j < columns.length; j++) {
        if (columns[j].extra == false) {
         continue;
        }
        gridContent += '		<p dataNo="' + i + '" columnNo="' + j + '" class="dlshouwen-grid-cell '
          + gridReflectionObj.getExtraColumnClassForHide(columns[j]) + '">';
      ...
      */
     /**
      * //绑定表头双击方法 if(gridReflectionObj.option.onHeaderDblClick){
      * $('#dlshouwen_grid_'+gridId+'
      * .dlshouwen-grid-header').dblclick(function(e){
      * gridReflectionObj.bindHeaderEvent(gridReflectionObj.option.onHeaderDblClick,
      * this, e); }); }
      */
     // 绑定表头鼠标滑过方法
    ...
    constructGridPageBar : function() {
     ...
     if (gridReflectionObj.exhibitDatas == null || gridReflectionObj.exhibitDatas.length <= 0) {
      ...
      if (typeof gridReflectionObj.option.pageSizeLimit == 'object') {
       pageSizeElement += '<select id="dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id
         + '" name="dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id + '" class="form-control change-page-size">';
       // 整合可用页码
       ...
       for (var i = 0; i < pageSizeLimit.length; i++) {
        ...
        pageSizeElement += '	<option ' + (pageSize == loopPageSize ? 'selected="selected"' : '') + ' value="' + loopPageSize + '">'
          + loopPageSize + '</option>';
       }
       ...
       // 如果是数值类型则为文本框
       pageSizeElement += '<input id="dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id
         + '" name="dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id
         + '" type="text" class="form-control change-page-size" value="' + gridReflectionObj.pager.pageSize + '" />';
       info = $.fn.dlshouwen.grid.tools.replaceAll(info, '{pageSize}', pageSizeElement);
      } else {
     ...
     // 绑定修改每页显示条数的事件
     $('#dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id).change(
       function() {
        var changePageSize = parseFloat(this.value);
        if (isNaN(changePageSize)) {
         $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.errors.notANumber, 'warning',
           3000);
        ...
        if (typeof gridReflectionObj.option.pageSizeLimit == 'number') {
         if (changePageSize > gridReflectionObj.option.pageSizeLimit) {
          $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.tools.replaceAll(
            $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.errors.maxPageSize, '{pageSizeLimit}',
            gridReflectionObj.option.pageSizeLimit), 'warning', 3000);
          $('#dlshouwen_grid_change_page_size_' + gridReflectionObj.option.id).val(gridReflectionObj.pager.pageSize);
          return;
         }
        }
     ...
     if (gridReflectionObj.exhibitDatas != null && gridReflectionObj.exhibitDatas.length > 0) {
      ...
      goFirstBtn.innerHTML = '<a href="javascript:void(0);">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.firstPage
        + '</a>';
      ...
      goPrevBtn.innerHTML = '<a href="javascript:void(0);">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.prevPage
        + '</a>';
      ...
      if (pageCount <= 5) {
       for (var i = 1; i <= pageCount; i++) {
        ...
        goPageBtn.title = $.fn.dlshouwen.grid.tools.replaceAll(
          $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.nowPageTitle, '{nowPage}', i);
        goPageBtn.className = i == nowPage ? 'active' : '';
        goPageBtn.innerHTML = '<a href="javascript:void(0);">'
          + $.fn.dlshouwen.grid.tools.replaceAll($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.nowPage, '{nowPage}', i)
          + '</a>';
      ...
      goNextBtn.innerHTML = '<a href="javascript:void(0);">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.nextPage
        + '</a>';
      ...
      goLastBtn.innerHTML = '<a href="javascript:void(0);">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].pageInfo.lastPage
        + '</a>';
    ...
    toolbar : function(type) {
     ...
     // 定义工具条内容
     var toolbars = {
      refresh : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.refreshTitle
        + '"><a href="javascript:void(0);" id="refreshGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.refresh + '</a></li>',
      fastQuery : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.fastQueryTitle
        + '"><a href="javascript:void(0);" id="fastQuery_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.fastQuery + '</a></li>',
      advanceQuery : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.advanceQueryTitle
        + '"><a href="javascript:void(0);" id="advanceQuery_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.advanceQuery + '</a></li>',
      exportExcel : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportExcelTitle
        + '"><a href="javascript:void(0);" id="exportExcelGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportExcel + '</a></li>',
      exportCsv : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportCsvTitle
        + '"><a href="javascript:void(0);" id="exportCsvGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportCsv + '</a></li>',
      exportPdf : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportPdfTitle
        + '"><a href="javascript:void(0);" id="exportPdfGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportPdf + '</a></li>',
      exportTxt : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportTxtTitle
        + '"><a href="javascript:void(0);" id="exportTxtGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.exportTxt + '</a></li>',
      print : '<li title="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.printTitle
        + '"><a href="javascript:void(0);" id="printGrid_replaceId">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.print + '</a></li>'
     };
    ...
    constructGridToolBar : function() {
     ...
     var gridReflectionObj = this;
     // 清空工具条
     $('#' + gridReflectionObj.option.toolbarContainer).html(
       '<span class="pagination pagination-sm dlshouwen-grid-tools"></span><span class="dlshouwen-grid-pager"></span>');
     // 如果为空则不做任何操作
     ...
     for (var i = 0; i < tools.split('|').length; i++) {
      ...
      if (tool == 'advanceQuery') {
       var content = $.fn.dlshouwen.grid.tools.replaceAll(gridReflectionObj.toolbar('advanceQuery'), 'replaceId',
         gridReflectionObj.option.id);
       $('#' + gridReflectionObj.option.toolbarContainer + ' .dlshouwen-grid-tools').append(content);
       // 绑定方法
      ...
      if (tool.indexOf('export') != -1) {
       ...
       for (var j = 0; j < tool.split(',').length; j++) {
        ...
        if (exportTool == 'excel') {
         content += $.fn.dlshouwen.grid.tools
           .replaceAll(gridReflectionObj.toolbar('exportExcel'), 'replaceId', gridReflectionObj.option.id);
        }
        if (exportTool == 'csv') {
    ...
    showProcessBar : function() {
     ...
     var content = '';
     content += '<div class="dlshouwen-grid-process-bar-top bg-primary" id="dlshouwen_grid_process_bar_top_' + gridReflectionObj.option.id
       + '"></div>';
     content += '<div class="dlshouwen-grid-process-bar-bottom bg-primary" id="dlshouwen_grid_process_bar_bottom_'
       + gridReflectionObj.option.id + '"></div>';
    ...
    getExtraColumnClassForHide : function(column) {
     ...
     if (column.hideType != undefined) {
      ...
      for (var i = 0; i < hideTypeArr.length; i++) {
       var type = hideTypeArr[i];
       if (type == 'lg')
        showClass += 'visible-lg ';
       if (type == 'md')
        showClass += 'visible-md ';
       if (type == 'sm')
        showClass += 'visible-sm ';
       if (type == 'xs')
    ...
    fastQuery : function() {
     ...
     var content = '';
     content += $.fn.dlshouwen.grid.tools.getWindowStart('fast_query_' + gridReflectionObj.option.id,
       $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.title);
      ...
      if (column.fastQuery == true) {
       ...
       if (column.fastQueryType == 'range') {
        ...
        if (column.type == 'date') {
         ...
         content += '	<input type="text" class="form-control" id="ge_' + column.id + '" name="ge_' + column.id + '" format="'
           + column.format + '" placeholder="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.selectStart
           + column.title + '" onclick="WdatePicker({dateFmt:\'' + $.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H')
           + '\'});" />';
         content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
         content += '</div>';
        } else {
         content += '<input type="text" class="form-control" id="ge_' + column.id + '" name="ge_' + column.id + '" placeholder="'
           + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.inputStart + column.title + '" />';
        }
        content += '</div>';
        content += '<div class="col-sm-4">';
        if (column.type == 'date') {
         content += '<div class="input-group">';
         content += '	<input type="text" class="form-control" id="le_' + column.id + '" name="le_' + column.id + '" format="'
           + column.format + '" placeholder="' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.selectEnd + column.title
           + '" onclick="WdatePicker({dateFmt:\'' + $.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H') + '\'});" />';
         content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
         content += '</div>';
        } else {
         content += '<input type="text" class="form-control" id="le_' + column.id + '" name="le_' + column.id + '" placeholder="'
           + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.inputEnd + column.title + '" />';
        }
        content += '</div>';
       } else if (column.codeTable) {
        content += '<div class="col-sm-4">';
        content += '	<select class="form-control" id="' + column.fastQueryType + '_' + column.id + '" name="' + column.fastQueryType + '_'
          + column.id + '">';
        content += '		<option value="">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.codeTableSelectAll
          + '</option>';
        ...
        if (column.type == 'date') {
         ...
         content += '	<input type="text" class="form-control" id="' + column.fastQueryType + '_' + column.id + '" name="'
           + column.fastQueryType + '_' + column.id + '" format="' + column.format + '" placeholder="' + hoderName + column.title
           + '" onclick="WdatePicker({dateFmt:\'' + $.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H') + '\'});" />';
         content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
         content += '</div>';
        } else {
         content += '<input type="text" class="form-control" id="' + column.fastQueryType + '_' + column.id + '" name="'
           + column.fastQueryType + '_' + column.id + '" placeholder="'
           + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.input + column.title + '" />';
        }
        content += '</div>';
       }
       content += '</div>';
     ...
     content += '</form>';
     var buttons = '';
     buttons += '<button type="button" class="btn btn-warning" id="resetFastQuery_' + gridReflectionObj.option.id + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.buttons.reset + '</button>';
     buttons += '<button type="button" class="btn btn-primary" id="doFastQuery_' + gridReflectionObj.option.id + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.buttons.query + '</button>';
    ...
    doFastQuery : function(form) {
     ...
     for (var i = 0; i < elements.length; i++) {
      ...
      } else if (($.fn.dlshouwen.grid.tools.equalsIgnoreCase(element.type, 'checkbox') || $.fn.dlshouwen.grid.tools.equalsIgnoreCase(
        element.type, 'radio'))
        && element.checked) {
       if (element.name != "") {
        gridReflectionObj.fastQueryParameters[element.name] = element.value;
    ...
    advanceQuery : function() {
     ...
     // 加载查询内容
     var content = '';
     content += $.fn.dlshouwen.grid.tools.getWindowStart('advance_query_' + gridReflectionObj.option.id,
       $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.title);
     content += '<div class="modal-body advance-query">';
     content += '	<ul class="nav nav-tabs" role="tablist">';
     // content += ' <li><a
     // href="#advance_query_plan_'+gridReflectionObj.option.id+'" role="tab"
     // data-toggle="tab">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.title+'</a></li>';
     content += '		<li class="active"><a href="#advance_query_condition_' + gridReflectionObj.option.id + '" role="tab" data-toggle="tab">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.title + '</a></li>';
     content += '		<li><a href="#advance_query_sort_' + gridReflectionObj.option.id + '" role="tab" data-toggle="tab">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.title + '</a></li>';
     content += '	</ul>';
     content += '	<div class="tab-content">';
     // content += ' <div class="tab-pane"
     // id="advance_query_plan_'+gridReflectionObj.option.id+'">';
     // content += ' <div class="panel panel-default">';
     // content += ' <input type="hidden"
     // id="advanceQueryId_'+gridReflectionObj.option.id+'"
     // name="advanceQueryId_'+gridReflectionObj.option.id+'" />';
     // content += ' <div class="form-horizontal" style="padding-top:15px;">';
     // content += ' <div class="form-group">';
     // content += ' <label class="col-sm-3 control-label
     // text-right">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.advanceQueryName+'</label>';
     // content += ' <div class="col-sm-6">';
     // content += ' <input type="text" class="form-control"
     // id="advanceQueryName_'+gridReflectionObj.option.id+'"
     // name="advanceQueryName_'+gridReflectionObj.option.id+'"
     // placeholder="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.advanceQueryNamePlaceHoder+'">';
     // content += ' </div>';
     // content += ' </div>';
     // content += ' <div class="form-group">';
     // content += ' <label class="col-sm-3 control-label
     // text-right">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.remark+'</label>';
     // content += ' <div class="col-sm-6">';
     // content += ' <textarea id="remark_'+gridReflectionObj.option.id+'"
     // name="remark_'+gridReflectionObj.option.id+'" class="form-control"
     // placeholder="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.remarkPlaceHoder+'"></textarea>';
     // content += ' </div>';
     // content += ' </div>';
     // content += ' <div class="form-group">';
     // content += ' <div class="col-sm-offset-3 col-sm-12">';
     // content += ' <button
     // id="addAdvanceQuery_'+gridReflectionObj.option.id+'"
     // title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.addAdvanceQueryTitle+'"
     // class="btn btn-xs btn-primary">';
     // content += '
     // '+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.addAdvanceQuery;
     // content += ' </button>';
     // content += ' <button
     // id="editAdvanceQuery_'+gridReflectionObj.option.id+'"
     // title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.editAdvanceQueryTitle+'"
     // class="btn btn-xs btn-primary">';
     // content += '
     // '+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.editAdvanceQuery;
     // content += ' </button>';
     // content += ' <button
     // id="copyAdvanceQuery_'+gridReflectionObj.option.id+'"
     // title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.copyAdvanceQueryTitle+'"
     // class="btn btn-xs btn-warning">';
     // content += '
     // '+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.copyAdvanceQuery;
     // content += ' </button>';
     // content += ' <button
     // id="deleteAdvanceQuery_'+gridReflectionObj.option.id+'"
     // title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.deleteAdvanceQueryTitle+'"
     // class="btn btn-xs btn-danger">';
     // content += '
     // '+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.deleteAdvanceQuery;
     // content += ' </button>';
     // content += ' </div>';
     // content += ' </div>';
     // content += ' </div>';
     ...
     content += '					<button id="insertConditionTr_' + gridReflectionObj.option.id + '" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.insertTitle
       + '" class="btn btn-xs btn-primary">';
     content += '						' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.insert;
     content += '					</button>';
     content += '					<button id="clearConditionTr_' + gridReflectionObj.option.id + '" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.clearTitle
       + '" class="btn btn-xs btn-danger">';
     ...
     content += '					<table id="conditionTable_' + gridReflectionObj.option.id
       + '" class="table table-bordered table-striped table-hover table-condition table-center" style="width:860px;">';
     content += '						<tr>';
     content += '							<th style="width:50px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.sequence + '</th>';
     content += '							<th style="width:100px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.operation + '</th>';
     content += '							<th style="width:80px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.leftParentheses + '</th>';
     content += '							<th style="width:140px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.field + '</th>';
     content += '							<th style="width:120px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.condition + '</th>';
     content += '							<th>' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.value + '</th>';
     content += '							<th style="width:80px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.rightParentheses + '</th>';
     content += '							<th style="width:90px;">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.logic + '</th>';
     content += '						</tr>';
     content += '					</table>';
     content += '				</div>';
     ...
     content += '			<div class="panel panel-default">';
     content += '				<div class="form-horizontal text-right advance-query-buttons">';
     content += '					<button id="insertSortTr_' + gridReflectionObj.option.id + '" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.insertTitle
       + '" class="btn btn-xs btn-primary">';
     content += '						' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.insert;
     content += '					</button>';
     content += '					<button id="clearSortTr_' + gridReflectionObj.option.id + '" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.clearTitle + '" class="btn btn-xs btn-danger">';
     content += '						' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.clear;
     content += '					</button>';
     content += '				</div>';
     content += '				<div class="advance-query-table-container">';
     content += '					<table id="sortTable_' + gridReflectionObj.option.id
       + '" class="table table-bordered table-striped table-hover table-condition table-center">';
     content += '						<tr>';
    ...
    doAdvanceQuery : function() {
     ...
     $('#conditionTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]')
       .each(
         function() {
          var id = this.id.split('_')[2];
          var seq = $('#conditionTable_' + gridReflectionObj.option.id + ' #seqTd_' + gridReflectionObj.option.id + '_' + id).html();
          var leftParentheses = $(
            '#conditionTable_' + gridReflectionObj.option.id + ' #leftParentheses_' + gridReflectionObj.option.id + '_' + id).val();
          ...
          var condition = $('#conditionTable_' + gridReflectionObj.option.id + ' #condition_' + gridReflectionObj.option.id + '_' + id)
            .val();
          var value = $('#conditionTable_' + gridReflectionObj.option.id + ' #value_' + gridReflectionObj.option.id + '_' + id).val();
          var rightParentheses = $(
            '#conditionTable_' + gridReflectionObj.option.id + ' #rightParentheses_' + gridReflectionObj.option.id + '_' + id).val();
          var logic = $('#conditionTable_' + gridReflectionObj.option.id + ' #logic_' + gridReflectionObj.option.id + '_' + id).val();
          if (field == '') {
           $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.tools.replaceAll(
             $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.errors.fieldMustSelect, '{sequence}', seq),
             'warning', 3000);
           pass = false;
           return false;
          }
          if (condition == '') {
           $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.tools.replaceAll(
             $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.errors.conditionMustSelect, '{sequence}', seq),
             'warning', 3000);
           pass = false;
           return false;
          }
     ...
     $('#sortTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]').each(
       function() {
        var id = this.id.split('_')[2];
        ...
        if (sortField == '') {
         $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.tools.replaceAll(
           $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.errors.fieldMustSelect, '{sequence}', seq), 'warning',
           3000);
         pass = false;
         return false;
        }
        if (sortLogic == '') {
         $.fn.dlshouwen.grid.tools.toast($.fn.dlshouwen.grid.tools.replaceAll(
           $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.errors.logicMustSelect, '{sequence}', seq), 'warning',
           3000);
         pass = false;
         return false;
        }
      ...
      if (advanceQueryParameter.advanceQueryConditions && advanceQueryParameter.advanceQueryConditions.length > 0) {
       ...
       for (var j = 0; j < advanceQueryParameter.advanceQueryConditions.length; j++) {
        ...
        if (advanceQueryCondition.condition == '5') {
         ...
         // 日期比较
         if (value instanceof Date) {
          condition += '(record["' + advanceQueryCondition.field + '"].getTime()-$.fn.dlshouwen.grid.tools.toDate("'
            + advanceQueryCondition.value + '", "' + advanceQueryCondition.format + '").getTime())>0';
         }
         // 数值比较
         if (!(value instanceof Date) && !isNaN(value)) {
       ...
       try {
        ...
        $.fn.dlshouwen.grid.tools.toast(
          $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.errors.conditionError, 'error', 3000);
    ...
    insertConditionTr : function() {
     ...
     content += '	<td>';
     content += '		<button type="button" id="moveUpConditionTr_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" class="btn btn-primary btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.upTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.up + '</button>';
     content += '		<button type="button" id="moveDownConditionTr_' + gridReflectionObj.option.id + '_'
       + gridReflectionObj.sequenceCondition + '" class="btn btn-primary btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.downTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.down + '</button>';
     content += '		<button type="button" id="deleteConditionTr_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" class="btn btn-danger btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.deleteTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons['delete'] + '</button>';
     content += '	</td>';
     content += '	<td>';
     content += '		<input type="text" id="leftParentheses_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" name="leftParentheses_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" class="form-control" />';
     content += '	</td>';
     content += '	<td>';
     content += '		<select id="field_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" name="field_'
       + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" class="form-control">';
     ...
     content += '		</select>';
     content += '		<input type="hidden" id="format_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" name="format_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" />';
     content += '	</td>';
     content += '	<td>';
     content += '		<select id="condition_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" name="condition_'
       + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" class="form-control">';
     content += '			<option value=""></option>';
     ...
     content += '	<td>';
     content += '		<input type="text" id="rightParentheses_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" name="rightParentheses_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition
       + '" class="form-control" />';
     content += '	</td>';
     content += '	<td>';
     content += '		<select id="logic_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" name="logic_'
       + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceCondition + '" class="form-control">';
     content += '			<option value=""></option>';
    ...
    moveUpConditionTr : function(id) {
     // 定义表格对象映像
     var gridReflectionObj = this;
     $('#conditionTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id).prev(
       '#conditionTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]').before(
       $('#conditionTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id));
     gridReflectionObj.resetConditionSeq();
    },
    // 下移
    moveDownConditionTr : function(id) {
     // 定义表格对象映像
     var gridReflectionObj = this;
     $('#conditionTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id).next(
       '#conditionTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]').after(
       $('#conditionTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id));
     gridReflectionObj.resetConditionSeq();
    },
    // 删除当前行
    ...
    fieldChange : function(id) {
     ...
     if (column.type == 'date') {
      var content = '';
      content += '<div class="input-group">';
      content += '	<input id="value_' + gridReflectionObj.option.id + '_' + id + '" name="value_' + gridReflectionObj.option.id + '_' + id
        + '" class="form-control" onclick="WdatePicker({dateFmt:\'' + $.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H')
        + '\'})" />';
      content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
      content += '</div>';
     ...
     if (column.codeTable) {
      var content = '';
      content += '<select id="value_' + gridReflectionObj.option.id + '_' + id + '" name="value_' + gridReflectionObj.option.id + '_' + id
        + '" class="form-control">';
      content += '	<option value=""></option>';
      for ( var key in column.codeTable) {
       content += '<option value="' + key + '">' + column.codeTable[key] + '</option>';
     ...
     // 其他则为默认文本
     var content = '<input id="value_' + gridReflectionObj.option.id + '_' + id + '" name="value_' + gridReflectionObj.option.id + '_' + id
       + '" class="form-control" />';
     $('#valueTd_' + gridReflectionObj.option.id + '_' + id).html(content);
    },
    // 条件触发
    ...
    insertSortTr : function() {
     ...
     content += '	<td>';
     content += '		<button type="button" id="moveUpSortTr_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort
       + '" class="btn btn-primary btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.upTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.up + '</button>';
     content += '		<button type="button" id="moveDownSortTr_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort
       + '" class="btn btn-primary btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.downTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.down + '</button>';
     content += '		<button type="button" id="deleteSortTr_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort
       + '" class="btn btn-danger btn-xs" title="'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.deleteTitle + '">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons['delete'] + '</button>';
     content += '	</td>';
     content += '	<td>';
     content += '		<select id="sortField_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort + '" name="sortField_'
       + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort + '" class="form-control">';
     content += '			<option value=""></option>';
     for (var i = 0; i < gridReflectionObj.option.columns.length; i++) {
      var column = gridReflectionObj.option.columns[i];
      if (column.advanceQuery != false) {
     ...
     content += '	</td>';
     content += '	<td>';
     content += '		<select id="sortLogic_' + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort + '" name="sortLogic_'
       + gridReflectionObj.option.id + '_' + gridReflectionObj.sequenceSort + '" class="form-control">';
     content += '			<option value=""></option>';
     content += '			<option value="0">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.logic.asc + '</option>';
     content += '			<option value="1">' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.logic.desc
       + '</option>';
     content += '		</select>';
     content += '	</td>';
     content += '</tr>';
    ...
    moveUpSortTr : function(id) {
     // 定义表格对象映像
     var gridReflectionObj = this;
     $('#sortTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id).prev(
       '#sortTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]').before(
       $('#sortTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id));
     gridReflectionObj.resetSortSeq();
    },
    // 下移
    moveDownSortTr : function(id) {
     // 定义表格对象映像
     var gridReflectionObj = this;
     $('#sortTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id).next(
       '#sortTable_' + gridReflectionObj.option.id + ' tr[id*=tr_' + gridReflectionObj.option.id + '_]').after(
       $('#sortTable_' + gridReflectionObj.option.id + ' #tr_' + gridReflectionObj.option.id + '_' + id));
     gridReflectionObj.resetSortSeq();
    },
    // 删除当前行
    ...
    print : function() {
     ...
     // 放置新的打印选项
     var content = '';
     content += $.fn.dlshouwen.grid.tools.getWindowStart('dlshouwen_grid_print_' + gridReflectionObj.option.id,
       $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.title);
     content += '				<table class="table table-bordered table-print">';
     content += '					<thead>';
     content += '						<tr>';
     content += '							<th><input type="checkbox" id="dlshouwen_grid_print_check_' + gridReflectionObj.option.id
       + '" checked="checked" /></th>';
     ...
     for (var i = 0; i < gridReflectionObj.option.columns.length; i++) {
      ...
      content += '						<td><input type="checkbox" id="dlshouwen_grid_print_check_' + gridReflectionObj.option.id + '_' + i
        + '" checked="checked" value="' + i + '" /></td>';
      content += '						<td>' + column.title + '</td>';
      content += '						<td>';
      content += '							<button type="button" class="btn btn-primary btn-xs" dataId="' + i + '" id="dlshouwen_grid_print_up_'
        + gridReflectionObj.option.id + '_' + i + '" title="'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.upTitle + '">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.up + '</button>';
      content += '							<button type="button" class="btn btn-primary btn-xs" dataId="' + i + '" id="dlshouwen_grid_print_down_'
        + gridReflectionObj.option.id + '_' + i + '" title="'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.downTitle + '">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.down + '</button>';
      content += '						</td>';
      content += '					</tr>';
     }
     content += '					</tbody>';
     ...
     // 绑定上移方法
     $('button[id*=dlshouwen_grid_print_up_' + gridReflectionObj.option.id + '_]').click(
       function() {
        var dataId = $(this).attr('dataId');
        var gridId = gridReflectionObj.option.id;
        $('#dlshouwen_grid_print_tr_' + gridId + '_' + dataId).prev(
          '#dlshouwen_grid_print_' + gridId + '_modal tr[id*=dlshouwen_grid_print_tr_' + gridId + ']').before(
          $('#dlshouwen_grid_print_tr_' + gridId + '_' + dataId));
       });
     // 绑定下移方法
     $('button[id*=dlshouwen_grid_print_down_' + gridReflectionObj.option.id + '_]').click(
       function() {
        var dataId = $(this).attr('dataId');
        var gridId = gridReflectionObj.option.id;
        $('#dlshouwen_grid_print_tr_' + gridId + '_' + dataId).next(
          '#dlshouwen_grid_print_' + gridId + '_modal tr[id*=dlshouwen_grid_print_tr_' + gridId + ']').after(
          $('#dlshouwen_grid_print_tr_' + gridId + '_' + dataId));
       });
     // 绑定打印方法
     $('#dlshouwen_grid_print_execute_' + gridReflectionObj.option.id).click(
       function() {
        // 画表格
        var content = '';
        content += '<table class="dlshouwen-grid ' + gridReflectionObj.option.tableClass + '" id="dlshouwen_grid_print_'
          + gridReflectionObj.option.id + '" style="' + gridReflectionObj.option.tableStyle + '">';
        if (gridReflectionObj.option.showHeader != false) {
         content += '<thead>';
         content += '	<tr>';
         $('input[id*=dlshouwen_grid_print_check_' + gridReflectionObj.option.id + '_]:checked').each(
           function() {
            var columnNo = this.value;
            content += '<th class="' + gridReflectionObj.option.columns[columnNo].headerClass + '" style="'
              + gridReflectionObj.option.columns[columnNo].headerStyle + '">' + gridReflectionObj.option.columns[columnNo].title + '</th>';
           });
         content += '	</tr>';
         content += '</thead>';
        }
        ...
        if (gridReflectionObj.exhibitDatas != null) {
         for (var i = 0; i < gridReflectionObj.exhibitDatas.length; i++) {
          ...
          $('input[id*=dlshouwen_grid_print_check_' + gridReflectionObj.option.id + '_]:checked').each(
            function() {
             var columnNo = this.value;
             content += '	<td class="' + gridReflectionObj.option.columns[columnNo].columnClass + '" style="'
               + gridReflectionObj.option.columns[columnNo].columnStyle + '">';
             if (gridReflectionObj.option.columns[columnNo].resolution) {
              content += gridReflectionObj.option.columns[columnNo].resolution(
                gridReflectionObj.exhibitDatas[i][gridReflectionObj.option.columns[columnNo].id], gridReflectionObj.exhibitDatas[i],
                gridReflectionObj.option.columns[columnNo], gridReflectionObj, i, columnNo);
             } else {
              content += gridReflectionObj.formatContent(gridReflectionObj.option.columns[columnNo],
                gridReflectionObj.exhibitDatas[i][gridReflectionObj.option.columns[columnNo].id]);
             }
             content += '	</td>';
            });
          content += '	</tr>';
    ...
    exportFile : function(exportType) {
     ...
     // 放置新的导出选项
     var content = '';
     content += $.fn.dlshouwen.grid.tools.getWindowStart('dlshouwen_grid_export_' + exportType + '_' + gridReflectionObj.option.id,
       $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].title);
     content += '				<div class="form-export modal-body form-horizontal form-export">';
     content += '					<div class="form-group">';
     content += '						<label class="col-sm-3 control-label">'
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].exportType.title + '</label>';
     content += '						<div class="col-sm-9">';
     content += '							<div class="checkbox">';
     content += '								<label><input type="radio" name="dlshouwen_grid_export_export_all_data_' + exportType + '_'
       + gridReflectionObj.option.id + '" value="0" checked="checked" /> '
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].exportType.now + '</label>';
     content += '								<label><input type="radio" name="dlshouwen_grid_export_export_all_data_' + exportType + '_'
       + gridReflectionObj.option.id + '" value="1" /> '
       + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].exportType.all + '</label>';
     content += '							</div>';
     content += '						</div>';
     content += '					</div>';
     content += '				</div>';
     ...
     content += '					<thead>';
     content += '						<tr>';
     content += '							<th><input type="checkbox" id="dlshouwen_grid_export_check_' + exportType + '_' + gridReflectionObj.option.id
       + '" checked="checked" /></th>';
     ...
     for (var i = 0; i < gridReflectionObj.option.columns.length; i++) {
      ...
      content += '						<td><input type="checkbox" id="dlshouwen_grid_export_check_' + exportType + '_' + gridReflectionObj.option.id + '_'
        + i + '" checked="checked" value="' + i + '" /></td>';
      content += '						<td>' + column.title + '</td>';
      content += '						<td>';
      content += '							<button type="button" class="btn btn-primary btn-xs" dataId="' + i + '" id="dlshouwen_grid_export_up_'
        + exportType + '_' + gridReflectionObj.option.id + '_' + i + '" title="'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].table.buttons.upTitle + '">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].table.buttons.up + '</button>';
      content += '							<button type="button" class="btn btn-primary btn-xs" dataId="' + i + '" id="dlshouwen_grid_export_down_'
        + exportType + '_' + gridReflectionObj.option.id + '_' + i + '" title="'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].table.buttons.downTitle + '">'
        + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang]['export'][exportType].table.buttons.down + '</button>';
      content += '						</td>';
      content += '					</tr>';
     }
     content += '					</tbody>';
     content += '				</table>';
     var buttons = '';
     buttons += '<button type="button" class="btn btn-primary" id="dlshouwen_grid_export_execute_' + exportType + '_'
       + gridReflectionObj.option.id + '">';
     ...
     // 绑定上移方法
     $('button[id*=dlshouwen_grid_export_up_' + exportType + '_' + gridReflectionObj.option.id + '_]').click(
       function() {
        var dataId = $(this).attr('dataId');
        var gridId = gridReflectionObj.option.id;
        $('#dlshouwen_grid_export_tr_' + exportType + '_' + gridId + '_' + dataId).prev(
          '#dlshouwen_grid_export_' + exportType + '_' + gridId + '_modal tr[id*=dlshouwen_grid_export_tr_' + exportType + '_' + gridId
            + ']').before($('#dlshouwen_grid_export_tr_' + exportType + '_' + gridId + '_' + dataId));
       });
     // 绑定下移方法
     $('button[id*=dlshouwen_grid_export_down_' + exportType + '_' + gridReflectionObj.option.id + '_]').click(
       function() {
        var dataId = $(this).attr('dataId');
        var gridId = gridReflectionObj.option.id;
        $('#dlshouwen_grid_export_tr_' + exportType + '_' + gridId + '_' + dataId).next(
          '#dlshouwen_grid_export_' + exportType + '_' + gridId + '_modal tr[id*=dlshouwen_grid_export_tr_' + exportType + '_' + gridId
            + ']').after($('#dlshouwen_grid_export_tr_' + exportType + '_' + gridId + '_' + dataId));
       });
     // 绑定导出方法
     $('#dlshouwen_grid_export_execute_' + exportType + '_' + gridReflectionObj.option.id)
       .click(
         function() {
          // 删除原表单
          ...
          gridPager.exportAllData = $(
            'input[name*=dlshouwen_grid_export_export_all_data_' + exportType + '_' + gridReflectionObj.option.id + ']:checked').val() == '1' ? true
            : false;
  ...
  tools : {
   ...
   getWindowStart : function(id, title) {
    var content = '';
    content += '<div class="modal fade" id="' + id + '_modal" tabindex="-1" role="dialog" aria-labelledby="' + id
   ...
   openToast : function(content, level, time) {
    ...
    var toastContent = '<div class="toast ' + levelClass + '" style="z-index:' + $.fn.dlshouwen.grid.tools.toastZIndex + '">' + content
      + '</div>';
   ...
   numberFormat : function(number, pattern) {
    ...
    if (/\./g.test(str)) {
     // 如果字符串有小数
     if (formatFloat != null) {
      var tempFloat = Math.round(parseFloat('0.' + str.split('.')[1]) * Math.pow(10, formatFloat.length))
    ...
    if (formatInt != null) {
     ...
     outputInt = outputInt.substr(0, outputInt.length % comma)
