...
(function() {
  ...
  dialog.onok = function() {
   ...
   case 'upload':
    ...
    if (count) {
     ...
     var confirmClose=confirm('您要放弃还未上传的文件吗?');
      if (confirmClose ) {
       if(list){
 ...
 RemoteImage.prototype = {
  ...
  setImage : function(img) {
   ...
   var wordImgFlag = img.getAttribute("word_img"), src = wordImgFlag ? wordImgFlag.replace("&amp;", "&") : (img.getAttribute('_src') || img
     .getAttribute("src", 2).replace("&amp;", "&")), align = editor.queryCommandValue("imageFloat");
  ...
  setPreview : function() {
   var url = $G('url').value, ow = $G('width').value, oh = $G('height').value, border = $G('border').value, title = $G('title').value, preview = $G('preview'), width, height;
   ...
   if (url) {
    preview.innerHTML = '<img src="' + url + '" width="' + width + '" height="' + height + '" border="' + border + 'px solid #000" title="'
      + title + '" />';
   }
  },
 ...
 UploadImage.prototype = {
  ...
  initUploader : function() {
   var _this = this, $ = jQuery, // just in case. Make sure it's not an other
                                 // libaray.
   $wrap = _this.$wrap,
   // 图片容器
   ...
   // WebUploader实例
   uploader, actionUrl = editor.getActionUrl(editor.getOpt('imageActionName')), acceptExtensions = (editor.getOpt('imageAllowFiles') || [])
     .join('').replace(/\./g, ',').replace(/^[,]/, ''), imageMaxSize = editor.getOpt('imageMaxSize'), imageCompressBorder = editor
   ...
   function addFile(file) {
    ...
    $btns = $(
      '<div class="file-panel">' + '<span class="cancel">' + lang.uploadDelete + '</span>' + '<span class="rotateRight">'
        + lang.uploadTurnRight + '</span>' + '<span class="rotateLeft">' + lang.uploadTurnLeft + '</span></div>').appendTo($li), $prgress = $li
      .find('p.progress span'), $wrap = $li.find('p.imgWrap'), $info = $('<p class="error"></p>').hide().appendTo($li),
   ...
   uploader.on('all', function(type, files) {
    ...
    case 'startUpload':
     /* 添加额外的GET参数 */
     var params = utils.serializeParam(editor.queryCommandValue('serverparam')) || '', url = utils.formatUrl(actionUrl
 ...
 OnlineImage.prototype = {
  ...
  pushData : function(list) {
   var i, item, img, icon, _this = this, urlPrefix = editor.getOpt('imageManagerUrlPrefix');
 ...
 SearchImage.prototype = {
  ...
  getImageData : function() {
   var _this = this, key = $G('searchTxt').value, type = $G('searchType').value, keepOriginName = editor.options.keepOriginName ? "1" : "0", url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word="
