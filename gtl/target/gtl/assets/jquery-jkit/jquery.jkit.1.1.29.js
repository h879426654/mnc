...
(function($, undefined) {
 ...
 // - **options** is either a string with a single command name or a JavaScript
 // object with all options or undefined
 // - **moreoptions** is optionally used in case options contains the a command
 // string and contains the options object

 $.jKit = function(element, options, moreoptions) {

  // Define all plugin defaults. These can be overwritten by the plugins options
  // set on init.
  ...
  var defaults = {
   ...
   // Now we set some default macros for often used command/parameter
   // combinations:
   ...
   // Next we're defining all the default options for each command. You can get
   // a good overview of them
   // on the official [cheat
   // sheet](http://jquery-jkit.com/pages/cheatsheet.php).
   ...
   commands : {
    ...
    // To make our code block funtionalllity not brake anything, we have to add
  ...
  // Set an alias to **this** so that we can use it everywhere inside our
  // plugin:
  ...
  // In case we are just applying a single command, we need to take the options
  // from the **moreoptions** parameter:
  ...
  // For a few things, we need some local plugin variables and objects, let's
  // set them now:
  ...
  // We want to know if the current window is in focus or not, we can do this
  // with the **window** object (just not in IE7 & 8):
  ...
  // The following collection of functions are internally used. There is a way
  // to call them with an external script,
  ...
  // ### init
  //
  // The **init** function is called on plugin init and sets up all the stuff we
  // need.

  plugin.init = function($el) {

   // In case this function is called without a specific DOM node, use the
   // plugins main DOM element:
   ...
   if (singlecommand != undefined) {
    ...
    // If this is an initialization of a single command, all we have to do is
    ...
    // It's now time to find all DOM nodes that want to execute a jKit command.
    // You can either use the **data-jkit** attribute,
    // or the **rel** attribute. **However, we strongly recommend to use the
    // data-jkit attribute!** The rel attribute support
    // will probably removed at some point.

    $el.find("*[rel^=jKit], *[" + s.dataAttribute + "]").each(
      function() {

       var that = this;

       // Get the rel or data-jkit attribute and extract all individual commands
       // (they have to be inside square brackets):
       ...
       $.each(relsplit, function(index, value) {
        ...
        // First convert all the escaped characters into internal jKit strings.
        // Later we convert them back and unescape them.

        value = value.replace(/\\=/g, '|jkit-eq|').replace(/\\:/g, '|jkit-dp|').replace(/\\;/g, '|jkit-sc|').replace(/\\\[/g, '|jkit-sbo|')
          .replace(/\\\]/g, '|jkit-sbc|').replace(/\\\*/g, '|jkit-st|').replace(/\\ /g, '|jkit-sp|');
        ...
        // It's still possible that this is a macro, just with changed options.
        // Let's check that and apply the macro if needed:
        ...
        // If this is a macro definition, add the current command string to the
        // macros array:
        ...
        if (options.type == 'macro' && relsplit[index - 1] != undefined) {
         ...
         // If this is the special repeat command, parse the options and than
         // add it to the delivered event handler:
         ...
         // Looks like this isn't one of the special use commands, so lets
         // execute one of the regular ones.

         // If the targets option is set, we first have to find out to which
         // target nodes we have to apply the command:
         ...
         $.each(targets, function(i, v) {
          ...
          // Now it's time to find out what the command key is for this specific
          // command call.
          // This can be set either by the commandkey option, the dot syntax or
          // if both are not
          // set, we take the elements id attribute or as a last option, we just
          ...
          // Now as we have the commandkey, we store it in the plugins
          // commandkey array together
          ...
          // Next we need to check if we have to immediately execute the command
          // or if we have to
          ...
          if (thisoptions.onevent === undefined) {
           ...
           // Check if a command replacement for this command is available and
           // if yes, call it.
  ...
  // ### parseOptions
  //
  // The **parseOptions** function takes a command string and creates an array
  // out of it with all options.
  // It automatically detects the command type and command key. An input string
  // can look like this
  ...
  // ### fixSpeed
  //
  // The **fixSpeed** function is used to make sure that a speed option has a
  ...
  // [command:myoption={rand|0-1000}]
  //
  // Currently only the random options are supported, but more stuff is planned,
  ...
  // ### getRandom
  //
  // The **getRandom** function simply generates a random number between a
  // minimum number and a maximum number.
  ...
  // ### findElementTag
  //
  // The **findElementTag** function makes it possible to find the tag name of a
  // specific element in a
  // previously defined structure. This makes it possible to write agnostic HTML
  // for tab or similar structures.
  // For example on the tab command, both this structures would be succesfully
  // detected:
  //
  // div (container)
  // div (element)
  ...
  // ### addDefaults
  //
  // The **addDefaults** function adds all the default options to the options
  // array. Additionally
  ...
  // ### executeCommand
  //
  // The **executeCommand** function is used to execute a command on a specific
  // DOM node with an array of options.
  ...
  plugin.executeCommand = function(that, type, options) {
   ...
   // Trigger the **jkit-commandinit** event on the main element with all useful
   // information attached to it.
   // This event is currently not used internally, but can of course be listened
   // to from outside the plugin.
   ...
   // Check if there is a limit set on how many times we're allowed to execute
   // this command (based on the command key)

   if (options.commandkey !== undefined) {
    commandkeys[options.commandkey]['execs']++;
    if ((options.execute == 'once' && commandkeys[options.commandkey]['execs'] > 1)
      || (!isNaN(options.execute) && commandkeys[options.commandkey]['execs'] > options.execute)) {
     return $that;
    }
   }
   ...
   $.each(options, function(i, v) {
    ...
    if (typeof v == 'string') {
     options[i] = v = v.replace(/\|jkit\-eq\|/g, '=').replace(/\|jkit\-dp\|/g, ':').replace(/\|jkit\-sc\|/g, ';').replace(/\|jkit\-sbo\|/g,
       '[').replace(/\|jkit\-sbc\|/g, ']').replace(/\|jkit\-st\|/g, '*').replace(/\|jkit\-sp\|/g, ' ');
    }
   ...
   // ##### Paginate Command
   //
   // The [paginate command](http://jquery-jkit.com/commands/paginate.html) lets
   // you create paginated content.
   ...
   case 'paginate':
    ...
    // Paginate has two ways to operate, either by node count or by actual
    // element height in pixels.
    // In the **node mode** we put a specific amount of DOM nodes into each
    // entry of the pages array.
    // In the **height mode** we actually measure the height of each element and
    // only put DOM nodes into the current page
    ...
    if (pages.length > 1) {
     ...
     // Now as we have the pages set up correctly and we actually have more than
     // one, it's time
     // to create the output DOM structure. The main element always gets the
     // page data and the
   ...
   // ##### Filter Command
   //
   // The [filter command](http://jquery-jkit.com/commands/filter.html) lets you
   // filter DOM nodes based on some input.

   case 'filter':

    // The filter has to run on init and after every change of elements that
    // have the **jkit-filter** class,
    // so we have our own function we can call on all those events, the
    // **filterElements** function, and
    ...
    // In case there's already a filter value set, we need to trigger the
    // filtering right now:
   ...
   // ##### Summary Command
   //
   // The [summary command](http://jquery-jkit.com/commands/summary.html)
   // creates a summary on specific content, for example the headers in a
   // content div.
   // The summary itself is either a linked list or a dropdown with automated
   // events.
   ...
   case 'summary':
    ...
    $(options.from).find(pre + options.what).each(function() {
     ...
     // If we're using all headers for our summary, than we have to do some
    ...
    if (output != '') {
     ...
     // In case this is a dropdown summary that is linked, we have to manually
     // add
     ...
     // And lastly if we create a select and have detected a hash, we need to
     // set that select to the correct value
   ...
   // ##### API Command
   //
   // The [API command](http://jquery-jkit.com/commands/api.html) lets you use
   // JSON based API's to display external data. Check the **plugin.readAPI**
   // for how it's done.
   ...
   // ##### Zoom Command
   //
   // The [zoom command](http://jquery-jkit.com/commands/zoom.html) makes it
   // possible to zoom into images on mouseover. To do that it
   // overlays the selected image with a div that has that image as its
   // background.
   ...
   // ##### Sort Command
   //
   // The [sort command](http://jquery-jkit.com/commands/sort.html) helps you
   // convert a normal table into a sortable table
   ...
   case 'sort':
    ...
    // First we need to know the exact position of the current TH inside the
    // heading TR, the index:
    ...
    $that.on('click', function() {
     ...
     // First we have to create an array with the content we need to base the
     // sort on:
     ...
     //
     // Depending on the current class of the TH, we either sort ascending or
     // descending.
     ...
     // Everything is ready, let's clear the whole TBODY of the table and add
     // the sorted rows:
   ...
   // ##### Ticker Command
   //
   // The [ticker command](http://jquery-jkit.com/commands/ticker.html) goes
   // through each item of a list and reveals
   ...
   case 'ticker':
    ...
    // Create an array with objects that contain all useful information of a
    // single ticker item:
    ...
    window.setTimeout(function() {
     plugin.ticker($newThat, options, messages, 0, 0);
    }, 0);
   ...
   // ##### Swap Command
   //
   // The [swap command](http://jquery-jkit.com/commands/swap.html) replaces a
   // DOM node attribute, for
   ...
   case 'swap':
    ...
    // We have to store the original attributes value to swap the attribute back
    // on mouseleave:

    var original = $that.attr(options.attribute);
    var replacement = $that.attr(options.attribute).replace(versions[0], versions[1]);

    // In case the attribute is an image source, we have to preload the image or
    // the swapping could have a delay:
   ...
   // ##### Fontsize Command
   //
   // The [fontsize command](http://jquery-jkit.com/commands/fontsize.html) can
   // be used to change the size of text.
   // It can be limited to specific elements. You can even use it to change
   // other CSS related sizes, for example the
   ...
   // ##### Cycle Command
   //
   // The [cycle command](http://jquery-jkit.com/commands/cycle.html) let's you
   // "cycle" through
   // a sequence of values and apply them to a set of DOM nodes. This can be
   // classes, html, attributes or css.
    ...
    $that.find(sel).each(function() {
     if (seq[pos] != undefined && seq[pos] != '') {
      ...
      default:
       ...
       // If it's not a class or html, it has to be a dot separated combination
       // like for exmaple
   ...
   // ##### Replace Command
   //
   // The [replace command](http://jquery-jkit.com/commands/replace.html) makes
   // it possible to replace content
   // based on a regex pattern. It acts on the HTML level, so not only text is
   // replacable!
   ...
   // ##### Ajax Command
   //
   // The [ajax command](http://jquery-jkit.com/commands/ajax.html) can do a few
   // thing. The normal use case is a link
   // that loads some extra content through an ajax call on click. But the
   // command can be used to lazy load images, too.

   case 'ajax':

    // If the href option is set, take it from the option, if not, take it from
    // our element:
    ...
    if (options.when == 'load' || options.when == 'viewport') {
     ...
     if (options.when == 'load') {
      ...
      // If it's **viewport**, we're going to wait till the content enters the
      // viewport before
   ...
   // ##### Key Command
   //
   // The [key command](http://jquery-jkit.com/commands/key.html) let's us
   // create hotkeys
   // for links. If thge link has an **onclick** attribute, we fire that one, if
   // not, we're just
   // going to open the href, either as a popup or inside the same window,
   // whatever the target
   // attribute tells us.
     ...
     $that.on(options.code, function() {
      if ($that.attr('onclick') !== undefined) {
       ...
       if ($that.attr('target') !== undefined && $that.attr('target') == '_blank') {
        ...
        // Sadly we can't open pages in a new tab or regular window, so we have
        // to open it in a popup instead:
   ...
   // ##### Live Command
   //
   // The [live command](http://jquery-jkit.com/commands/live.html) is very
   // simple. All it does is reload
   // the source of an image or iframe in a certain interval and making sure
   // that it doesn't load the cashed version.
   ...
   // ##### Split Command
   //
   // The [split command](http://jquery-jkit.com/commands/split.html) can take a
   // string, for example a comma separeted one,
   // and create new HTML elements out of the individual parts. This way a
   // simple comma separated list can be transformed
   ...
   // ##### Encode Command
   //
   // The [encode command](http://jquery-jkit.com/commands/encode.html) apply
   // various encodings to the content of an element.
   // If the option is code, the content is not only HTML encoded, it can even
   // remove the extra tab whitespace you get if you
   ...
   // ##### Chart Command
   //
   // The [chart command](http://jquery-jkit.com/commands/chart.html) let's us
   // create simple horizontal bar charts using
   // different sized divs. This is definitely a good candidate for a command
   // replacement using the canvas element to draw
   // different charts.
   ...
   case 'chart':
    ...
    // Set the maximum value for our chart either from the options or from all
    // the data values:
    ...
    // Now loop through each data label and add the data from each plot to it.
    // We are using a delay for each
    // plot and increase that delay with each new data label. This way we get a
    // nice animation where every plot
    ...
    $.each(datalabels, function(i, v) {
     ...
     $.each(plots, function(i2, v2) {
      ...
      if (plots[i2].data[i] > 0) {
       ...
       var $plot = $('<div/>', {
        'class' : s.prefix + '-' + type + '-plot ' + s.prefix + '-' + type + '-plot' + i2
       }).appendTo($step);
    ...
    // Evyerything is set up, so replace the original element with our new
    // chart:
   ...
   // ##### Lightbox Command
   //
   // The [lightbox command](http://jquery-jkit.com/commands/lightbox.html) can
   // be used to overlay a
   // bigger version of an image on click, content in an overlayed iframe or to
   // display a modal dialog
   // box above the content.

   case 'lightbox':

    // First we need to find out what the source is we're going to display in
    // the lightbox.
    // If the href is set, we take that one, if not we check the src attribute
    // and if that's
    ...
    if (src != '') {
     ...
     // A lightbox can be part of a group of lightbox content, for example to
     // display a gallery of image.
     // To achive this feature, we're using an array that is set on plugin init
     // and available thoughout
     // the whole plugin.
     ...
     $that.on('click', function() {
      ...
      // If this is not a modal window, we have to create an overlay that
      // darkens the
      // whole content:
      ...
      // We need another DIV for the content that's placed right above the
      // overlay:
      ...
      // iOS devices need a small hack to display the content correctly in case
      // the
      // page is scrolled:
      ...
      // If there's a fixed width or height set in the options, we have to
      ...
      // Time to create the DOM nodes that contain the navigational elements and
      // close button:
      ...
      // In case this one is part of a group, we need to create the navigation
      // and
      // bind all the needed events to it. Both, the left and the right
      // navigation isn't
      ...
      // The last element we have to create and poistion corrently is the
      ...
      // Because IE is a stupid browser and doesn't fire the load element
      // correctly in older versions
      // if the image is already in the cash, we have to force load a new
      ...
      // Time to load the image or iframe content. The little trick here is to
      // always try to load
      // an image, even if there isn't one supplied, because this way we can use
      // the error callback
      // to find out if we actually have an image or not. As soon as the load
      // event has fired, we
      // can get the width an height and will be able to calculate all the
      // placement and scaling
      // information we need.

      var img = new Image();
      // fix: image may changed.
      var srcThat = '';
      if ($that.attr('href') !== undefined)
       srcThat = $that.attr('href');
      if (srcThat == '' && $that.attr('src') !== undefined)
       srcThat = $that.attr('src');
      if (srcThat == '' && $that.css('background-image') !== undefined) {
       srcThat = $that.css('background-image').replace('"', '').replace('"', '').replace('url(', '').replace(')', '');
      }
      src = srcThat;
      // }
      $(img).load(function() {
      ...
      }).attr('src', src).appendTo($content).error(
        function() {
         $content.html(
           '<iframe id="' + s.prefix + '-' + type + '-iframe" src="' + src + '" style="border:none;width:100%;height:100%"></iframe>')
           .fadeTo(options.speed, 1);
        });
   ...
   // ##### Scroll Command
   //
   // The [scroll command](http://jquery-jkit.com/commands/scroll.html) let's us
   // scroll smoothly to
   // an anchor on the page or if the HREF attribute is empty, we just scroll to
   // the top.
   ...
   case 'scroll':
    ...
    $that.click(function() {
     ...
     // The dynamic option changes the scroll animation duration based on the
     // distance between
     ...
     // Finally animate the **scrollTop** of the whole HTML page to scroll
   ...
   // ##### Hide Command
   //
   // The [hide command](http://jquery-jkit.com/commands/hide.html) is used to
   // hide an element, animated or not.
   ...
   // ##### Remove Command
   //
   // The [remove command](http://jquery-jkit.com/commands/remove.html) is used
   ...
   // ##### Show Command
   //
   // The [show command](http://jquery-jkit.com/commands/show.html) is used to
   // reveal an element, animated or not.
   ...
   //
   // The [showandhide
   // command](http://jquery-jkit.com/commands/showandhide.html) is used to
   // reveal an element
   ...
   // ##### Loop Command
   //
   // The [loop command](http://jquery-jkit.com/commands/loop.html) does the sam
   // thing as the **showandhide** command,
   // but repeats itself again, and again ... actually, pretty much forever.
   // **Oh, and lease don't use it as a blink tag!!!**
   ...
   // ##### Random Command
   //
   // The [random command](http://jquery-jkit.com/commands/random.html) can be
   // used to randomly select a
   // specific amount of elements from a collection of elements. All not
   // selected ones will either be hidden
   ...
   // let's us display an element
   // only partially in case it is bigger than our supplied maximum height. The
   // whole height is shown only
   ...
   case 'partially':
    ...
    // Only add the magic is we need, in other words, if the lement is higher
    // than our maximum height:

    if (originalHeight > options.height) {

     // We can only add our *more div* if it's relatively positioned, so force
     // that one on it:
   ...
   // ##### Slideshow Command
   //
   // The [slideshow command](http://jquery-jkit.com/commands/slideshow.html) is
   ...
   case 'slideshow':
    ...
    if (options.on != '') {
     ...
     // Set the correct events and use a setTimeout function to call the
     // slideshow function:
     ...
     $that.on(options.on, function() {
      if (options.on == 'click') {
       if ($.data($that, 'anim')) {
        ...
        $.data($that, 'anim', true);
        window.setTimeout(function() {
         plugin.slideshow(slides, 0, $that, options);
        }, 0);
       }
   ...
   // ##### Carousel Command
   //
   // The [carousel command](http://jquery-jkit.com/commands/carousel.html) is
   // used to display a
   ...
   case 'carousel':
    ...
    // First hide all elements that are over our limit of elements we want to
    // show:
    ...
    // Add our **prev** and **next** button elements so the user can control the
    // carousel:
   ...
   // has two uses. Either it can be used
   // to animate the CSS of an element or it can be used to animated a kind of
   // keyframe animation with attribute
   // tweenings.
   ...
   case 'animation':
    ...
    if (options.to != '') {
     ...
     setTimeout(function() {
      ...
      // {!} task: Dublicate code and the delay doesn't make much sense with an
      // event like this.
     ...
     window.setTimeout(function() {
      plugin.animation(frames, -1, $that, options);
     }, 0);
   ...
   // ##### Gallery Command
   //
   // The [gallery command](http://jquery-jkit.com/commands/gallery.html) takes
   // a bunch of images and creates
   // a gallery out of it.
   ...
   case 'gallery':
    ...
    // Create the element that will contain the thumbnails and insert it after
    // the gallery:
    ...
    // Now loop through all images and add them to the thumbnail div. Add the
    // correct events and
   ...
   // ##### Tabs Command
   //
   // The [tabs command](http://jquery-jkit.com/commands/tabs.html) is used to
   // create a tab navigation
   // of different content elements.

   case 'tabs':

    // First try to find out which kind of HTML elements are used to structure
    // the data:
    ...
    // Next we need to create an array the contains all the tab titles and
    // content:
    ...
    // We need a jQuery element right now but can only set it after the
    // following loop has finished:
    ...
    $.each(tabs, function(index, value) {
     ...
     // {!} task: Was there a reason for this callback variable? Can't we just
     // use an anonymos function inside the event?
    ...
    // Do we have to display an initial content or do we start without a tab
    // selected?
   ...
   case 'accordion':
    ...
    // Next we need to create an array that contains all the titles and content
    // (yes, this
    // is so similar to the tabs command, that even the array is called "tabs"):

    // {!} task: Can we save code if we combine this one and the tab command?
    ...
    // Loop through each element and create the correct data structure with all
   ...
   // ##### Parallax Command
   //
   // The [parallax command](http://jquery-jkit.com/commands/parallax.html) is
   // used to create a parallax scrolling
   // effect with different layers that look like a 3D scenery. Sidescrolling
   // games in the old days used this kind
   ...
   case 'parallax':
    ...
    // We have to attach our event to different DOM elements based on the the
    // type of parallax we want.
    // The first option will use window scrolling to set the position of each
    // layer, the other two use
    ...
    $capture.on(options.detect,
      function(event) {

       // We only want to go through all calculations if needed, so check if our
       // element is inside the viewport
       // and that the window has focus:

       if ((windowhasfocus || !windowhasfocus && s.ignoreFocus)
         && ($that.jKit_inViewport() || !$that.jKit_inViewport() && s.ignoreViewport)) {
        var cnt = 1;
        ...
        // Loop through each layer and set the correct positions of each one of
        // them:
   ...
   // ##### Menu Command
   //
   // The [menu command](http://jquery-jkit.com/commands/menu.html) adds some
   // additional features
   // to a CSS based menu.
   ...
   // ##### Form Command
   //
   // The [form command](http://jquery-jkit.com/commands/form.html) can convert
   // a regular web form into
   // an ajax submitted form. Additionally it adds various validation options.

   case 'form':

    // Add a hidden field that will contain a list of required fileds so that
    // our PHP script can check
    // against them.
    ...
    // Add an on submit event so that we can do our work before the form is
    // being submitted:
    ...
    $that.on('submit',
      function() {
       ...
       // {!} task: Can't we use the default parsing here so that we save code
       // and get all features?

       $(this).find("*[rel^=jKit], *[" + s.dataAttribute + "]")
         .each(
           function() {
            ...
            // Check if this form element is required and if yes, check if there
            ...
            if ((required || $(this).val() != '') || options.type == 'group') {
             ...
             // min (numeric)?
             if ((options.type == 'int' || options.type == 'float') && options.min != undefined && $(this).val() < options.min
               && options.type != 'group') {
              elerror = true;
              errors.push({
               'element' : $(this),
             ...
             // max (numeric)?
             if ((options.type == 'int' || options.type == 'float') && options.max != undefined && $(this).val() > options.max
               && options.type != 'group') {
              elerror = true;
              errors.push({
               'element' : $(this),
             ...
             if ((options.type == 'int' || options.type == 'float') && options.bigger != undefined
               && $(this).val() > $(options.bigger).val()) {
              elerror = true;
              errors.push({
               'element' : $(this),
               'error' : 'bigger'
             ...
             if ((options.type == 'int' || options.type == 'float') && options.smaller != undefined
               && $(this).val() < $(options.smaller).val()) {
              elerror = true;
              errors.push({
               'element' : $(this),
               'error' : 'smaller'
             ...
             // min (length)?
             if ((options.type != 'int' && options.type != 'float') && options.min != undefined && $(this).val().length < options.min
               && options.type != 'group') {
              elerror = true;
              errors.push({
               'element' : $(this),
             ...
             // max (length)?
             if ((options.type != 'int' && options.type != 'float') && options.max != undefined && $(this).val().length > options.max
               && options.type != 'group') {
              elerror = true;
              errors.push({
               'element' : $(this),
             ...
             if ((options.type != 'int' && options.type != 'float') && options.longer != undefined
               && $(this).val().length > $(options.longer).val().length) {
              elerror = true;
              errors.push({
               'element' : $(this),
               'error' : 'longer'
             ...
             if ((options.type != 'int' && options.type != 'float') && options.shorter != undefined
               && $(this).val().length < $(options.shorter).val().length) {
              elerror = true;
              errors.push({
               'element' : $(this),
               'error' : 'shorter'
       ...
       if (errors.length == 0) {
        ...
        // If this form doesn't use an ajax submit, than just fire the
        // "complete" event:
        ...
        if (options.validateonly == "yes") {
           ...
           function(data, textStatus, jqXHR) {
            ...
            if (data.sent != undefined && data.sent == true) {
             ...
             for (x in data.error) {
              ...
              $that.find('*[name=' + field + ']').addClass(s.errorClass).after(
                '<span class="' + s.errorClass + '">' + options.error + '</span>');
             }
             plugin.triggerEvent('error', $that, options);
            }

            // Something didn't really work. Is there even a compatible form
            // script? Show error:
         ...
         // Return **false** so that the browser doesn't submit the form
         // himself:
   ...
   // ##### Plugin Command
   //
   // The [plugin command](http://jquery-jkit.com/commands/plugin.html) makes it
   // possible to add jQuery plugins
   ...
   case 'plugin':
    ...
    if (options.script != undefined) {
     ...
     $.getScript(options.script, function() {
      ...
      // by calling the correct function name with the correct set of
      // parameters:
   ...
   case 'tooltip':
    ...
    $that.on('mouseenter', function(e) {
     ...
     // Has this tooltip custom styling either by class or by supplying color
     // values?
     ...
     // Fix the tooltip position so that we don't get tooltips we can't read
   ...
   // that is scaled to the full width and height of the window, either skewed
   // or just zoomed.

   case 'background':

    // Create a background element with a negative z-index that is scaled to the
   ...
   // ##### Lorem Command
   //
   // The [lorem command](http://jquery-jkit.com/commands/lorem.html) adds lorem
   // ipsum text or paragraphs to
   ...
   // ##### Binding Command
   //
   // The [binding command](http://jquery-jkit.com/commands/binding.html) lets
   // you bind different
   // kind of values and even functions to different things, for example
   // attributes. It's a very
   // powerfull command with tons of options.

   case 'binding':

    window.setTimeout(function() {
     plugin.binding($that, options);
    }, 0);
   ...
   // ##### Limit Command
   //
   // The [limit command](http://jquery-jkit.com/commands/limit.html) either
   // limts the
   // characters of a string or the elements inside a container by a set number.

   case 'limit':

    // Limit the number of elements. Set speed to zero if you want to hide them
    // immediately
    // or use an animation:
  ...
  // ### filterElements
  //
  // The **filterElements** function is used by the filter command. It's doing
  // the heavy lifting for the command.

  plugin.filterElements = function($el, options) {

   // First we need to go through each filter element to find out by what we
   // have to filter the affected
   // elemnts. We're splitting each elements value to get the separate words
   // into an array.
   ...
   // Where do we have to look for our affected DOM nodes? Inside the body or
   ...
   // Loop through all affected elements and check if they have to be displayed
   // or not.

   $container.find(options.affected).each(function() {

    // First create a few cashes to speed up the following loop through the
    // filter selections:
    ...
    if (selections.length > 0) {
     ...
     // As we have two options, one where we need to find each selection and one
     // where we
     // only have to find one of the selections, we first have to create an
     // array with all
     // found selections:
   ...
   // Fire the complete event at the right time and hide the optional loader
   // animation:
  ...
  // ### readAPI
  //
  // The **readAPI** function is used by the API command. It is currently able
  // to read JSONP APIs and
   ...
   if (options.format == 'json') {
    ...
    $.ajax({
     ...
     success : function(data) {
      ...
      // If a template is supplied in the options, use it to display the data we
      // just received:
      ...
      if (options.template != '') {
       ...
       // Than we add the data to each element that has the **data-jkit-api**
       // attribute:
       ...
       // And lastly, we remove all elements that have the **data-jkit-api-if**
       // attribute, but
       ...
       // In case we don't use a template, just add the specified value as text
       // to the element:
      ...
      // Do we have to read the API in a specific interval? If yes, set a
      // timeout:
  ...
  // ### triggerEvent
  //
  // The **triggerEvent** function is used by various commands to trigger an
  // event on an element with
  ...
  // ### cssFromString
  //
  // The **cssFromString** function is used by the animation command. It parses
  // a specially formated string
  // and creates an object that contains all CSS data. Here's an exmaple of the
  // string format:
  //
  ...
  // ### preFix
  //
  // The **preFix** function is used by the encode command to remove unneaded
  // tab stops from
  ...
  plugin.ticker = function($el, options, messages, currentmessage, currentchar) {
   ...
   // We only run the ticker animation if the element is inside the viewport and
   // the window in focus:

   if ((windowhasfocus || !windowhasfocus && plugin.settings.ignoreFocus)
     && ($el.jKit_inViewport() || !$el.jKit_inViewport() && plugin.settings.ignoreViewport)) {

    var timer = options.speed;
    currentchar++;

    // Check if we're at the end of the current ticker message. If yes, start
    ...
    if (currentchar > messages[currentmessage].text.length) {
     ...
     // We are still on the same message, so just display the current amaount of
     // characters, either inside a link or
     // as text:

    } else {
     if (messages[currentmessage].href != undefined) {
      $el.html('<a href="' + messages[currentmessage].href + '" target="' + messages[currentmessage].target + '">'
        + messages[currentmessage].text.substr(0, currentchar) + '</a>');
     } else {
   ...
   if (!stopped) {
    window.setTimeout(function() {
     plugin.ticker($el, options, messages, currentmessage, currentchar);
    }, timer);
   }
  ...
  // ### loadAndReplace
  //
  // The **loadAndReplace** function is used by the ajax command. It basically
  // loads an urls full data and than
  // looks for a specific element isnide that data. It than replaces that
  // element with the same element on the
  // current page.

  plugin.loadAndReplace = function(href, options, $el) {

   // Create an unique temporary id we can use to store and access our loaded
   // content.
   ...
   $(options.element).jKit_effect(false, options.animation, options.speed, options.easing, 0, function() {
    ...
    // Prepare the current element and create a div we use to store the loaded
    // content:
    ...
    // Load the content from the supplied url and tell jQuery which element we
    // need from it:

    $('#' + tempid).load(href + ' ' + options.element, function() {

     // Add the content from our temporary div to our real element and
  ...
  // ### updateSrc
  //
  // The **updateSrc** function is used by the live command. It changes the src
  // url in a way that forces the browser
  ...
  // ### applyTemplate
  //
  // The **applyTemplate** function is used by the template command. It adds the
  // template with all it's
  // options to the suoplied element.

  plugin.applyTemplate = function($el, options, cnt, entries) {

   // Loop through each template variable, find the correct element inside the
   // supplied content element,
   // init each of the elements in case there are any jKit commands on them and
   // finally store the value or HTML
   // in an array.
   ...
   // Hide all elements that have an **if-entry-x** class. We later show them
   // again if needed.
  ...
  //
  // The **renameDynamicAttributes** function is used by the
  // **plugin.applyTemplate** function. It's
  // used to rename attributes on dynamic elements so that we get unique
  // elements.
  ...
  // ### binding
  //
  // The **binding** function is used by the binding command. It connects
  // different things to other things.
  ...
  plugin.binding = function(el, options) {
   ...
   if (windowhasfocus || !windowhasfocus && plugin.settings.ignoreFocus) {
    ...
    // First we need to get our value or multiple values that we later convert
    // into one.
    // Only run the following code if we didn't bind to a function already.

    if (options.value == undefined) {

     // If the selector option is set, we don't use a variable for our source,
     // that means
     // we have to get our variable from the selector element:

     if (options.selector != '') {

      // The selector can have multiple elements that we have to check. And the
      // source option
      ...
      // Now create an array that holds one or all of our values and run through
      // each selector:

      var values = [];
      $.each(selsplit, function(i, v) {

       // The selector is either a jQuery selector string or "this" that
       // references the
       // current element or "parent" that references the parent of the current
       // element.
       ...
       // A jQuery selector string can match more than one string, so run
       // through all of them:

       $(v).each(function() {

        // The source option defines from where exactly we get our value. There
        // are quite
        // a few options.

        switch (sourcesplit[0]) {

        // We can trigger this binding by an event. This will simply call this
        // function again
        ...
        // "attr" will get a specific attribute that is specified after the dot,
        // for example
        // **attr.id**:
        ...
        // "css" will get a specific css value that is specified after the dot,
        // for example
        // **css.width**. Many of the options will be calculated by jQuery so
        // that we get
        ...
        // "scroll" will get the page scroll offset, either from top or from the
        // left side:
        ...
        // "clearance" will calculate the clearence around an element in
        // relation to the window. If
        ...
        // "has" tries to find out if the element has a specific thing and gives
        // back either
        // true or false:
        ...
        // "location" needs the second option after the dot and gets something
        // from the location
      ...
      // If there's a third option supplied with the source, for example
      // **css.height.max**, we use
      // that to calculate the final valus from all values in the array. If the
      // fird option isn't
    ...
    // In case we have a numeric value, there are a few options more we can
    // apply. The **accuracy**
    // option reduces the accuracy of the value down to this number. The **min**
    // and **max** options
    ...
    // If the **condition** option is set or the current value is a boolean, we
    // have to decide if either the
    // **if** option will be used (if supllied) or the value in the **else**
    // option (if supllied).
    ...
    // Next we add some logic that either fires the **true** or **false** event
    // in case our value changes.
    // To make this possible, we store the value in the "global" commandkeys
    // array entry for the current command call.
    ...
    // Time to use the value to set something, but only if the condition wasn't
    // false:
    ...
    if (doit) {
     ...
     // The **mode** option defines what we have to do with our value. There are
     // quite a few options.
     // The **attr** and **css** modes take a second option that is separated
     // with a dot.
     ...
     default:
      ...
      // The default behavior is to call a custom function if one exits with
      // that name:
  ...
  // ### loop
  //
  // The **loop** function is used by the loop command and basically just shows
  // and hides and element and than starts
  // the next interval.

  plugin.loop = function($that, options) {

   if ((windowhasfocus || !windowhasfocus && plugin.settings.ignoreFocus)
     && ($that.jKit_inViewport() || !$that.jKit_inViewport && plugin.settings.ignoreViewport)) {
  ...
  // ### scaleFit
  //
  // The **scaleFit** function is used by the background command. It calculates
  // the correct
  // with, height of the image relative to the window and the correct position
  // inside that
  ...
  plugin.scaleFit = function(bg, element, originalWidth, originalHeight, distort) {
   ...
   // If we don't want to distort the image, we now have to do some additional
   // calculations:
  ...
  // ### carousel
  //
  // The **carousel** function is used by the carousel command. It moves the
  // carousel either one forward, or
  // one backward.
  ...
  plugin.carousel = function($el, options, dir) {
   ...
   // Only run the carousel if we're inside the viewport and the window has
   // focus:
  ...
  // ### slideshow
  //
  // The **slideshow** function is used by the slideshow command. It replaces
  // one slide with the next one. This one is
  // is really simple, so not much to comment, just that the old element is
  // first being hidden and than the new one shown,
    ...
    if ((windowhasfocus || !windowhasfocus && plugin.settings.ignoreFocus)
     ...
     el.jKit_effect(false, options.animation, options.speed, options.easing, 0, function() {
      ...
      el.jKit_effect(true, options.animation, options.speed, options.easing, 0, function() {
       window.setTimeout(function() {
        plugin.slideshow(slides, current, el, options);
       }, options.interval);
      });
     });
  ...
  // ### animation
  //
  // The **animation** function is used by the animation command, but only in
  // case it's a keyframe animation.
  ...
  plugin.animation = function(frames, current, el, options) {
   ...
   if ((windowhasfocus || !windowhasfocus && plugin.settings.ignoreFocus)
    ...
    // Loop through each frame and run the frames action in case it matches the
    // current frame number:

    $.each(frames, function(index, value) {
     if (value.start == current) {

      // First add the new element by cloning it from the frames object and
      ...
      // Depending on the action that is set for this frame, we need to start
    ...
    if ((nextloop && options.loop == "yes") || !nextloop) {
     window.setTimeout(function() {
      plugin.animation(frames, current, el, options);
     }, options.interval);
    }
  ...
  // ### closeLightbox
  //
  // The **closeLightbox** function is used by the lightbox command to close the
  // active lightbox programmatically
  ...
  // ### addKeypressEvents
  //
  // The **addKeypressEvents** function is used by the key command and adds a
  // specific keycode event
  // to an element.
 ...
 // ### jKit_effect
 //
 // The **jKit_effect** plugin function is used by all kind of jKit commands
 // that perform animations.
 ...
 // ### jKit_getUnixtime
 //
 // The **jKit_getUnixtime** plugin function returns an unix timestamp based on
 // the current date.
 ...
 // ### jKit_arrayShuffle
 //
 // The **jKit_arrayShuffle** plugin function is used to shuffle an array
 // randomly.
 ...
 //
 // The **jKit_stringOccurrences** plugin function is used to count the times a
 // string is found inside
 // another string.
 ...
 // ### jKit_emailCheck
 //
 // The **jKit_emailCheck** plugin function is used by the validation command to
 // check if an
 ...
 // ### jKit_urlCheck
 //
 // The **jKit_urlCheck** plugin function is used by the validation command to
 // check if an
 // url is valid.
 ...
 // ### jKit_dateCheck
 //
 // The **jKit_dateCheck** plugin function is used by the validation command to
 // check if the
 ...
 // ### jKit_timeCheck
 //
 // The **jKit_timeCheck** plugin function is used by the validation command to
 // check if the
 ...
 // ### jKit_phoneCheck
 //
 // The **jKit_phoneCheck** plugin function is used by the validation command to
 // check if the
 ...
 //
 // The **jKit_passwordStrength** plugin function is used by the validation
 // command to check if the
 // password strength is good enough. The function calculates a score from 0 to
 // 100 based on various
 // checks.
 ...
 // ### jKit_getAttributes
 //
 // The **jKit_getAttributes** plugin function returns an array with all
 // attributes that are
 ...
 // ### jKit_setAttributes
 //
 // The **jKit_setAttributes** plugin function creates a set of supplied
 // attributes on an emelemnt.
 ...
 // ### jKit_iOS
 //
 // The **jKit_iOS** plugin function checks the user agent if the current device
 // runs iOS.
 ...
 // ### jKit_belowTheFold
 //
 // The **jKit_belowTheFold** plugin function checks if the supplied element is
 // below the page fold.
 ...
 // ### jKit_aboveTheTop
 //
 // The **jKit_aboveTheTop** plugin function checks if the supplied element is
 // above the top of the currently visible part of the page.
 ...
 // ### jKit_rightOfScreen
 //
 // The **jKit_rightOfScreen** plugin function checks if the supplied element is
 // right from the current voiewport.
 ...
 // ### jKit_leftOfScreen
 //
 // The **jKit_leftOfScreen** plugin function checks if the supplied element is
 // left from the current voiewport.
 ...
 // ### jKit_inViewport
 //
 // The **jKit_inViewport** plugin function checks if the supplied element is
 // inside the viewport.
