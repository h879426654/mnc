...
<c:set var="pattern" value="^[a-z\d_]*((\d_?[a-z])|([a-z]_?\d))[a-z\d_]*$" />
<c:set var="pattern_msg" value="${_title}不符合规则(必须是数字和字母的组合)" />
...
<input type="password" name="${_name}" class="form-control" pattern="${pattern}" data-pattern-msg="${pattern_msg}"
