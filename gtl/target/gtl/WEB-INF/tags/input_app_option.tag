...
<%@attribute name="_parentId" type="java.lang.String" required="true"%>
...
</c:if> name="${_name}" data-role="dropdownlist"
 data-value-field="${_value_field}" data-text-field="${_text_field}" <c:if test="${not empty _filter}">
data-filter="${_filter}"
</c:if>
 class="form-control"
 data-source="
