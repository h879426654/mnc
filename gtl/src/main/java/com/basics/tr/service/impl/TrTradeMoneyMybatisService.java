...
@Service
@Transactional
public class TrTradeMoneyMybatisService extends GenericMybatisService<TrTradeMoney> implements TrTradeMoneyService {

	@Autowired
	private TrTradeMoneyDao trTradeMoneyDao;
	 ...
	 */
	@Override
	public FormResultSupport doApplyTrade(TrTradeMoney entity) {
		FormResultSupport result = new FormResultSupport();
		// 判断当前交易是否为待审核状态
		TrTradeMoney info = trTradeMoneyDao.queryOne(new QueryFilterBuilder().put("id", entity.getId()).put("applyStatus", Constant.APPLY_STATUS_1).build());
		if (null != info) {
	 ...
	 */
	@Override
	public FormResultSupport doApplyTradeFail(TrTradeMoney entity) {
		FormResultSupport result = new FormResultSupport();
		// 判断当前交易是否为待审核状态
		TrTradeMoney trade = trTradeMoneyDao.queryOne(new QueryFilterBuilder().put("id", entity.getId()).put("applyStatus", Constant.APPLY_STATUS_1).build());
		if (null != trade) {
			// 解冻出售方货币
			...
			if (null != account) {
				...
				// 流水
				doAddCustomerProfit(account.getId(), trade.getId(), trade.getLockMoneyNum(), Constant.PROFIT_STATUS_1, Constant.MONEY_TYPE_1, "交易审核不通过");
			}
