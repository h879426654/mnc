...
@Component
@Transactional
public class EveryDayAutoConfirmOrderJob extends BaseApiService implements EveryDayJob {

	@Override
	public void doJob() {
		LogUtils.performance.info("{} begin at {}", this.getClass().getName(), new Date());
		List<OrOrder> orders = orOrderDao.query(new QueryFilterBuilder().put("orderStatus", Constant.ORDER_STATUS_3).put("orderConfirm",new Date()).build());
