...
public class ImageInfo {
  ...
  * names as parameters (information on them will be printed to standard output,
  * one line per file) or call it with no parameters. It will then check data
 ...
 private static void printCompact(String sourceName, ImageInfo imageInfo) {
  ...
  System.out.println(sourceName + SEP + imageInfo.getFormatName() + SEP + imageInfo.getMimeType() + SEP + imageInfo.getWidth() + SEP
   + imageInfo.getHeight() + SEP + imageInfo.getBitsPerPixel() + SEP + imageInfo.getNumberOfImages() + SEP + imageInfo.getPhysicalWidthDpi()
   + SEP + imageInfo.getPhysicalHeightDpi() + SEP + imageInfo.getPhysicalWidthInch() + SEP + imageInfo.getPhysicalHeightInch() + SEP
  ...
  * rather time-consuming task. Not all file formats support more than one
  * image. If this method is called with <code>true</code> as argument, the
  * actual number of images can be queried via {@link #getNumberOfImages()}
